...
use tokio_rusqlite::{Row, Rows};

use crate::{Error, Result};

/// Iterator to automatically deserialize each row from owned `tokio_rusqlite::Rows` into `D: serde::Deserialize`
...
/// Iterator to automatically deserialize each row from borrowed `tokio_rusqlite::Rows` into `D: serde::Deserialize`
...
#[inline]
fn deser_row<D: DeserializeOwned>(
    row: tokio_rusqlite::Result<Option<&Row>>,
    columns: &Option<Vec<String>>,
) -> Option<Result<D>> {
...
fn columns_from_rows(rows: &tokio_rusqlite::Rows) -> Option<Vec<String>> {
    rows.as_ref().map(|stmt| {
        ...
        for i in 0..len {
            out.push(
                stmt.column_name(i)
                    .expect("Impossible, we checked the length")
                    .to_owned(),
            )
        }
        out
