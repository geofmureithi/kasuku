...
fn make_connection_with_spec(table_spec: &str) -> rusqlite::Connection {
    ...
    con.execute(&format!("CREATE TABLE test({})", table_spec), [])
        .unwrap();
    con
}

fn test_value_same<
    T: serde::Serialize + serde::de::DeserializeOwned + PartialEq + Debug + Clone,
>(
    db_type: &str,
    src: &T,
) {
...
fn test_ser_err<S: serde::Serialize, F: Fn(&Error) -> bool>(value: &S, err_check_fn: F) {
    match super::to_params(value) {
        Err(e) => assert!(
            err_check_fn(&e),
            "Error raised was not of the correct type, got: {}",
            e
        ),
...
fn test_values_with_cmp_fn<S, D, F>(
    db_type: &str,
    value_ser: &S,
    value_de: &D,
    comparison_fn: Option<F>,
) where
    S: serde::Serialize,
...
fn test_uint() {
    ...
    test_value_same(
        "INT CHECK(typeof(test_column) == 'integer')",
        &98169812698712987_u64,
    );
    test_ser_err(&u64::MAX, |err| {
        matches!(*err, super::Error::ValueTooLarge(..))
    });
}
...
fn test_float() {
    ...
    test_value_same(
        "REAL CHECK(typeof(test_column) == 'real')",
        &f64::NEG_INFINITY,
    );
...
fn test_tuple() {
    ...
    // deserialization with columns
    {
        ...
        let mut res = stmt
            .query_and_then([], |row| {
                super::from_row_with_columns::<Test>(row, &columns)
            })
            .unwrap();
        assert_eq!(res.next().unwrap().unwrap(), src);
    }
...
fn test_struct() {
...

    {
        ...
        assert_eq!(res.next().unwrap().unwrap(), src);
        assert_eq!(
            super::from_row::<Test>(rows.next().unwrap().unwrap()).unwrap(),
            src
        );
    }
...
fn test_attrs() {
    ...
    let mut stmt = con
        .prepare("SELECT f_real, f_blob, f_null FROM test")
        .unwrap();
    {
