...
use std::rc::{Rc, Weak};
use std::{fmt, mem};
use types::{Event, PluginEvent, Rsx};
...
impl Render<Node> for Node {
    fn render_into(self: Box<Self>, parent: &Node) -> Result<(), hirola::prelude::Error> {
...
mod node_type_serde {
        ...
        S: Serializer,
    {
        match &val.ty.as_ref() {
            NodeType::Element(t) => {
                CustomNodeType::serialize(&CustomNodeType::Element(t.borrow().clone()), s)
            }
            NodeType::Comment(t) => {
                CustomNodeType::serialize(&CustomNodeType::Comment(t.borrow().clone()), s)
            }
            NodeType::Text(t) => {
                CustomNodeType::serialize(&CustomNodeType::Text(t.borrow().clone()), s)
            }
            NodeType::Fragment(t) => {
                CustomNodeType::serialize(&CustomNodeType::Fragment(t.borrow().clone()), s)
            }
        }
    }
...
impl TryFrom<Node> for Rsx {
    ...
    fn try_from(dom: Node) -> Result<Self, Self::Error> {
        Ok(Rsx(serde_json_wasm::to_string(&dom)
            .map_err(|e| types::Error::Serialization(e.to_string()))?))
    }
}

impl TryFrom<Rsx> for Node {
    type Error = types::Error;

    fn try_from(rsx: Rsx) -> Result<Self, Self::Error> {
        serde_json_wasm::from_str(&rsx.0).map_err(|e| types::Error::Serialization(e.to_string()))
    }
}

pub fn emit<Ev: Serialize + PluginEvent>(data: &Ev) -> types::Event {
    Event {
        namespace: std::any::type_name::<Ev::Plugin>().to_string(),
        data: serde_json_wasm::to_vec(data).unwrap(),
    }
}
